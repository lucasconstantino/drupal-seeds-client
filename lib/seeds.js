...
function SeedsClassGenerator(seedsEndpoint) {
   ...
   * @param {string} Runtime type. Either 'each' or 'all' for now.
   */
  Seeds.prototype.attach = function (runtime) {
    var seeds = this;
    runtime = typeof runtime !== 'undefined' ? runtime : 'all';

    var attachmentRuntimes = {
      'all': {
        'createAll': GLOBAL.beforeAll,
        'removeAll': GLOBAL.afterAll
      },
      'each': {
        'createAll': GLOBAL.beforeEach,
        'removeAll': GLOBAL.afterEach
      }
    };

    if (attachmentRuntimes[runtime]) {
      Object.keys(attachmentRuntimes[runtime]).forEach(function (method) {
        attachmentRuntimes[runtime][method](function () {
          return seeds[method]();
        });
      });
    }
  ...
  /**
   * Testing flow helper to attach seeds creation/removal to a
   * test using Jasmine/Protractor format.
   */
  Seeds.prototype.it = function (body) {
    return function () {
      return this.createAll().then(function (values) {
        var promise = Q.Promise(function (resolve, reject) {
          var result = body.apply(this, [values, resolve]);

          // Handle returning promise.
          if (result && result.then) return result.then(resolve);

          // Handle no asynchronicity.
          if (body.length < 3) protractor.promise.controlFlow().execute(resolve);
        });

        return promise.finally(this.removeAll.bind(this));
      }.bind(this));
    }.bind(this);
  };
